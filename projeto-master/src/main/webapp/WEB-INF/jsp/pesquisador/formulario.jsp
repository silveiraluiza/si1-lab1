		...
		<div class="container div-ident">
			<div class="col-md-9 div-ident">
				...
				<form role="form" id="form" class=""
					...
					<div class="form-group">
						<label class="control-label">Lista de Departamento: </label> <select
							name="pesquisador.departamento.id">
							<option value="">Escolha o Departamento</option>
							<c:forEach items="${departamento}" var="departamento">
								<option value="${departamento.id}"
									<c:if test="${departamento.id == departamento2.id}">Select</c:if>>>${departamento.sigla}
					...
					<div class="form-group">
						<label class="control-label">Lista de Categorias: </label> <select
							name="pesquisador.categoria.id">
							<option value="">Escolha a Categoria</option>
							<c:forEach items="${categoria}" var="categoria">
								<option value="${categoria.id}"
									<c:if test="${categoria.id == categoria2.id}">Select</c:if>>>${categoria.tipo}</option>
							</c:forEach>
						</select>
					</div>
